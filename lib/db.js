...
function DB(source) {
    ...
    this._ioManager = new IOManager(this._adapter);
...
DB.prototype.create = function () {
    return this._adapter.init().then(() => {
        return generateId();
    }).then((id) => {
        const out = this._ioManager.createTemporary(id);
...
DB.prototype.get = function (id) {
    ...
    return this._idSet.includes(id).then((includes) => {
        ...
        return new Item(id, this._ioManager);
    });
};
